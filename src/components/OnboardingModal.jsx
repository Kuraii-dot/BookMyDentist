...
import { X, MapPin, Calendar, Star, Shield, ChevronRight, CheckCircle2 } from 'lucide-react'
    ...
    content: (
      <div className="space-y-4">
        ...
        <div className="bg-amber-50 border border-amber-200 rounded-xl p-3">
          <p className="text-amber-700 text-xs font-medium">💡 Tip: You can search by service name — try "Braces", "Cleaning", or "Whitening"</p>
        </div>
      </div>
    ),
  },
      ...
      <div className="space-y-4">
        <div className="bg-slate-50 rounded-xl p-4 border border-slate-200 space-y-3 text-xs text-slate-600 leading-relaxed">
          <div className="flex items-start gap-2">
            ...
            <p><strong>Booking Platform Only.</strong> BookMyDentistPH is not a medical provider. We connect patients with dental clinics. All medical decisions rest with you and your dentist.</p>
          </div>
          <div className="flex items-start gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5"/>
            <p><strong>Verified Clinics.</strong> All clinics are reviewed by our admin team before listing. However, we encourage you to read reviews and do your own research.</p>
          </div>
          <div className="flex items-start gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5"/>
            <p><strong>Your Privacy.</strong> We take your data seriously. Your personal information is never sold to third parties. See our full Privacy Policy for details.</p>
          </div>
          <div className="flex items-start gap-2">
            <CheckCircle2 className="w-4 h-4 text-emerald-500 shrink-0 mt-0.5"/>
            <p><strong>Cancellations.</strong> You can cancel upcoming appointments through your dashboard. Please be considerate of clinic schedules.</p>
          </div>
        </div>
...
export default function OnboardingModal() {
  ...
  useEffect(()=>{
    ...
    if (!seen) {
      setTimeout(()=>setOpen(true), 800) // small delay feels more natural
    }
  },[])
      ...
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden animate-fade-in">
        ...
        <div className="flex items-start justify-between px-6 pt-5 pb-1">
          <div className="flex items-center gap-3">
            <div className="w-11 h-11 rounded-2xl bg-sky-50 border border-sky-100 flex items-center justify-center text-2xl shrink-0">
              {current.icon}
            </div>
            <div>
