          ...
          <form onSubmit={handleSaveProfile} className="space-y-4">
            ...
            <div className="flex items-center gap-4">
              <div className="relative group cursor-pointer" onClick={()=>document.getElementById('avatar-input').click()}>
                <div className="w-18 h-18 w-16 h-16 rounded-xl overflow-hidden bg-sky-100 border-2 border-white shadow-md ring-2 ring-sky-200">
                  {avatarUrl
            ...
            <Field label="Mobile Number" hint={phoneVerified
              ? '✅ Phone verified — SMS notifications active'
              : form.phone ? '⚠️ Not verified — go to Phone tab to verify' : 'Add your number to receive SMS booking alerts'}>
        ...
        <div className="card p-6">
          {phoneVerified ? (
            <div className="text-center py-4">
              ...
              <div className="bg-sky-50 rounded-xl p-4 text-left space-y-1.5 border border-sky-100">
                <p className="section-label mb-2">SMS notifications include</p>
                {['📅 Appointment confirmations','⏰ Reminders (1 day before)','❌ Cancellations or reschedules'].map(s=>(
            ...
            <div>
              <h3 className="font-display font-bold text-slate-800 text-base mb-1">Verify your mobile number</h3>
              <p className="text-slate-500 text-sm mb-5">We'll send a 6-digit code via SMS.</p>
              {phoneStep==='idle' && (
                <div className="space-y-4">
                  ...
                  <button onClick={handleSendOtp} disabled={sendingOtp||!form.phone} className="btn btn-primary btn-md w-full">
                    {sendingOtp ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"/>Sending...</> : '📱 Send Code'}
                  </button>
                </div>
              )}
              {phoneStep==='sent' && (
                <div className="space-y-4">
                  ...
                  <button onClick={handleVerifyOtp} disabled={verifyingOtp||otpCode.replace(/\s/g,'').length<6}
                    className="btn btn-primary btn-md w-full">
                    {verifyingOtp ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"/>Verifying...</> : 'Verify Code'}
                  </button>
                  ...
                  <button onClick={()=>{setPhoneStep('idle');setOtpCode('')}}
                    className="w-full text-center text-slate-400 text-sm hover:text-slate-600">← Change number</button>
                </div>
              )}
            </div>
          )}
      ...
      {tab==='security' && (
        <div className="card p-6">
          <form onSubmit={handleChangePassword} className="space-y-4">
            <Alert type="info">Enter your new password twice to confirm the change.</Alert>
            ...
            <button type="submit" disabled={savingPw||pwForm.newPass!==pwForm.confirm} className="btn btn-primary btn-md w-full">
              {savingPw ? <><span className="w-4 h-4 border-2 border-white/40 border-t-white rounded-full animate-spin"/>Updating...</> : 'Update Password'}
            </button>
          </form>
        </div>
      )}
