...
export default function ProfileEdit() {
  ...
  async function handleSaveProfile(e) {
    ...
    if (avatarFile) {
      ...
      const { error: uploadError } = await supabase.storage.from('clinic-images').upload(path, avatarFile, { upsert: true })
      if (uploadError) { toast.error('Failed to upload avatar'); setSaving(false); return }
  ...
  return (
    <div className="max-w-lg">
      ...
      <div className="flex gap-1 bg-white rounded-2xl border border-slate-100 p-1 mb-6 shadow-sm">
        {[{ key: 'profile', label: '👤 Profile' }, { key: 'password', label: '🔒 Password' }].map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 py-2.5 rounded-xl text-sm font-semibold transition-all ${tab === t.key ? 'bg-teal-600 text-white shadow-sm' : 'text-slate-500 hover:text-slate-700'}`}>
            {t.label}
          </button>
        ))}
      </div>
          ...
          <form onSubmit={handleSaveProfile} className="space-y-5">
            ...
            <div className="flex items-center gap-5">
              <div className="relative group cursor-pointer" onClick={() => document.getElementById('avatar-input').click()}>
                <div className="w-20 h-20 rounded-2xl overflow-hidden bg-teal-100 border-2 border-white shadow-md">
                  {avatarUrl
              ...
              <div>
                ...
                <button type="button" onClick={() => document.getElementById('avatar-input').click()}
                  className="btn btn-secondary btn-sm mt-2">Change Photo</button>
              </div>
            ...
            <div>
              ...
              <input type="email" value={profile?.email || ''} disabled className="input bg-slate-50 text-slate-400 cursor-not-allowed" />
        ...
        <div className="card p-6">
          <form onSubmit={handleChangePassword} className="space-y-4">
            <div className="bg-teal-50 border border-teal-100 rounded-xl p-3 mb-2">
              <p className="text-teal-700 text-xs">For security, enter your new password twice to confirm the change.</p>
            </div>
            <div>
