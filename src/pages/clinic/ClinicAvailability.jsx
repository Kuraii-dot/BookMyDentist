...
import {
  Clock, Calendar, Settings, Umbrella, Save,
  Plus, X, AlertTriangle, CheckCircle2, Info,
  Sun, Coffee, Zap,
} from 'lucide-react'
...
const TABS = [
  ...
  { key: 'dates',    label: 'Date Overrides', Icon: Calendar  },
  { key: 'settings', label: 'Booking Rules',  Icon: Settings  },
  { key: 'vacation', label: 'Vacation Mode',  Icon: Umbrella  },
]
  ...
  return (
    <div className="flex flex-col gap-1">
      ...
      <input type="time" value={value} onChange={e => onChange(e.target.value)}
        className="input !w-auto !py-1.5 text-sm" style={{ minWidth: '110px' }} />
    </div>
  )
}
...
export default function ClinicAvailability() {
  ...
  function addSpecialDate() {
    ...
    if (newSpecialOpen >= newSpecialClose) { toast.error('Opening must be before closing'); return }
  ...
  return (
    <div className="max-w-2xl">
      <div className="page-header flex items-start justify-between gap-4 flex-wrap">
        ...
        {vacationMode && (
          <span className="badge badge-warning text-sm px-3 py-1.5 flex items-center gap-1.5">
            <Umbrella className="w-3.5 h-3.5" /> Vacation Mode ON
          </span>
        )}
      </div>
      ...
      <div className="flex gap-1 card p-1 mb-6 rounded-2xl overflow-x-auto">
        {TABS.map(t => (
          <button key={t.key} onClick={() => setTab(t.key)}
            className={`flex-1 min-w-max flex items-center justify-center gap-1.5 py-2.5 px-3 rounded-xl text-sm font-semibold transition-all whitespace-nowrap
        ...
        <div className="animate-fade-in space-y-4">
                    ...
                    {s.enabled ? (
                      <div className="flex items-end gap-2 flex-wrap flex-1">
                        ...
                        <div className="mb-1.5">
                          {timeError
                            ? <span className="text-xs text-red-400 font-medium flex items-center gap-1"><AlertTriangle className="w-3 h-3" />Close must be after open</span>
                            : slots > 0
                              ? <span className="text-xs text-slate-400">~{slots} slot{slots !== 1 ? 's' : ''}</span>
                              : <span className="text-xs text-amber-400 flex items-center gap-1"><Info className="w-3 h-3" />No slots — adjust times or duration</span>
                          }
                        </div>
                      </div>
                    ) : (
          ...
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-semibold text-slate-800">Appointment Slot Duration</h3>
            </div>
            <p className="text-slate-400 text-xs mb-3">How long each slot is — determines how many fit in your schedule</p>
          ...
          <div className="card p-5">
            <div className="flex items-center justify-between mb-3">
              <div className="flex items-center gap-2">
                ...
                <div>
                  ...
                  <p className="text-slate-400 text-xs mt-0.5">No slots generated during break</p>
                </div>
              </div>
            ...
            {hasBreak && (
              <div className="flex items-end gap-2 flex-wrap mt-2">
                ...
                {breakStart >= breakEnd && (
                  <span className="text-xs text-red-400 mb-2 flex items-center gap-1">
                    <AlertTriangle className="w-3 h-3" />End must be after start
                  </span>
                )}
              </div>
            )}
        ...
        <div className="animate-fade-in space-y-5">
          ...
          <div className="card p-5">
            <div className="flex items-center gap-2 mb-1">
              ...
              <h3 className="font-semibold text-slate-800">Special Open Dates</h3>
            </div>
            <p className="text-slate-400 text-xs mb-4">Override hours for a specific date — open on a closed day, or with different hours</p>
              ...
              <div className="border border-slate-200 rounded-xl p-4 mb-4 space-y-3">
                ...
                <div className="flex items-end gap-2 flex-wrap">
                  ...
                  {newSpecialOpen >= newSpecialClose && newSpecialDate && (
                    <span className="text-xs text-red-400 mb-2 flex items-center gap-1">
                      <AlertTriangle className="w-3 h-3" />Close must be after open
                    </span>
                  )}
                </div>
                  ...
                  <div key={s.date} className="flex items-center gap-3 rounded-xl px-4 py-2.5 border"
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <p className="text-xs" style={{ color: 'var(--color-brand-text)' }}>
                        {fmt12(s.open)} – {fmt12(s.close)}{s.note && ` · ${s.note}`}
                      </p>
                    </div>
      ...
      {tab === 'settings' && (
        <div className="animate-fade-in space-y-4">
          ...
          <div className="card p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-400 text-xs mt-0.5 mb-3">Useful if multiple dentists see patients simultaneously</p>
          ...
          <div className="card p-5">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1">
                ...
                <p className="text-slate-400 text-xs mt-0.5 mb-3">How far ahead a patient must book</p>
                ...
                <div className="flex items-center gap-2 mt-1">
                  ...
                  <input type="number" min="0" max="168" step="0.25" value={advanceNoticeHours}
                ...
                <p className="text-slate-400 text-xs mt-2 flex items-center gap-1">
                  ...
                  {advanceNoticeHours === 0
                    ...
                    : advanceNoticeHours < 1
                      ? `Patients must book at least ${advanceNoticeHours * 60} minutes in advance`
                      : advanceNoticeHours < 24
                        ? `Patients must book at least ${advanceNoticeHours} hour${advanceNoticeHours > 1 ? 's' : ''} in advance`
                        : `Patients must book at least ${advanceNoticeHours / 24} day${advanceNoticeHours > 24 ? 's' : ''} in advance`}
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="card p-5">
            <div className="flex items-start gap-4">
              <div className="w-10 h-10 rounded-xl flex items-center justify-center shrink-0" style={{ backgroundColor: 'var(--color-brand-light)' }}>
                <Calendar className="w-5 h-5" style={{ color: 'var(--color-brand)' }} />
              </div>
              <div className="flex-1">
                <h3 className="font-semibold text-slate-800">Booking Window</h3>
                <p className="text-slate-400 text-xs mt-0.5 mb-3">How far ahead patients can schedule</p>
        ...
        <div className="animate-fade-in space-y-4">
          <div className={`card p-6 border-2 transition-all ${vacationMode ? 'border-amber-300' : 'border-slate-100'}`}>
            <div className="flex items-center justify-between gap-4 mb-4">
              <div className="flex items-center gap-3">
                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center ${vacationMode ? 'bg-amber-100' : 'bg-slate-100'}`}>
                  <Umbrella className={`w-6 h-6 ${vacationMode ? 'text-amber-500' : 'text-slate-400'}`} />
                </div>
                <div>
            ...
            {vacationMode && (
              <div className="space-y-4 pt-4 border-t border-amber-100 animate-fade-in">
                <div className="bg-amber-50 border border-amber-200 rounded-xl p-3 flex items-start gap-2">
                  ...
                  <p className="text-amber-700 text-xs font-medium">While vacation mode is ON, patients cannot book new appointments. Existing bookings are not affected.</p>
                </div>
                ...
                <div>
                  ...
                  <textarea value={vacationMessage} onChange={e => setVacationMessage(e.target.value)} rows={3}
                    placeholder="e.g. We're on vacation until March 15. We'll be back soon!"
      ...
      <div className="mt-6 sticky bottom-4">
        <button onClick={handleSave} disabled={saving} className="btn btn-primary btn-lg w-full rounded-2xl shadow-lg flex items-center justify-center gap-2">
          {saving
