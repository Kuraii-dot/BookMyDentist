        ...
        <div className="absolute bottom-0 left-4 translate-y-1/2">
          <div className="w-10 h-10 rounded-xl overflow-hidden border-2 border-white bg-white shadow-sm">
            {clinic.logo_url
              ...
              : <div className="w-full h-full flex items-center justify-center bg-sky-50 text-base">🦷</div>
            }
          </div>
        </div>
        {clinic.avg_rating>0 && (
...
export default function BrowseClinics() {
  ...
  useEffect(()=>{
    supabase.from('clinics').select('*, services(id,name,price), reviews(rating)')
      .eq('is_active',true).eq('verification_status','approved').order('created_at',{ascending:false})
      .then(({data})=>{
