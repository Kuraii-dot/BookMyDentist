...
import {
  Calendar, Clock, ShieldCheck, BarChart2, AlertCircle,
  CheckCircle2, XCircle, RefreshCw, Trophy, Ban,
  Building2, ChevronRight, Search
} from 'lucide-react'

const STATUS_CONFIG = {
  pending:             { label: 'Pending',     class: 'badge-warning', Icon: AlertCircle,   iconClass: 'text-amber-500 animate-pulse' },
  accepted:            { label: 'Confirmed',   class: 'badge-success', Icon: CheckCircle2,  iconClass: 'text-emerald-500' },
  rejected:            { label: 'Declined',    class: 'badge-danger',  Icon: XCircle,       iconClass: 'text-red-500' },
  rescheduled:         { label: 'Rescheduled', class: 'badge-info',    Icon: RefreshCw,     iconClass: 'text-blue-500 animate-spin' },
  reschedule_accepted: { label: 'Confirmed',   class: 'badge-success', Icon: CheckCircle2,  iconClass: 'text-emerald-500' },
  reschedule_declined: { label: 'Declined',    class: 'badge-danger',  Icon: XCircle,       iconClass: 'text-red-500' },
  completed:           { label: 'Completed',   class: 'badge-purple',  Icon: Trophy,        iconClass: 'text-violet-500' },
  cancelled:           { label: 'Cancelled',   class: 'badge-gray',    Icon: Ban,           iconClass: 'text-slate-400' },
}
...
export default function CustomerDashboard() {
  ...
  const statItems = [
    { label:'Upcoming',  value: upcoming.length,                                        Icon: Calendar,     iconClass:'text-sky-500',    bg:'bg-sky-50',    border:'border-sky-100',    anim:'group-hover:scale-110' },
    { label:'Completed', value: appointments.filter(a=>a.status==='completed').length,   Icon: Trophy,       iconClass:'text-violet-500', bg:'bg-violet-50', border:'border-violet-100', anim:'group-hover:scale-110' },
    { label:'Cancelled', value: cancelled.length,                                        Icon: Ban,          iconClass:'text-slate-400',  bg:'bg-slate-100', border:'border-slate-200',  anim:'' },
    { label:'Total',     value: appointments.length,                                     Icon: BarChart2,    iconClass:'text-amber-500',  bg:'bg-amber-50',  border:'border-amber-100',  anim:'group-hover:scale-110' },
  ]
    ...
    <div className="animate-fade-in space-y-6">
      ...
      <div className="relative overflow-hidden rounded-3xl gradient-banner p-6 text-white">
        ...
        <div className="relative flex items-center justify-between gap-4 flex-wrap">
          ...
          <Link to="/dashboard/browse" className="btn btn-secondary btn-md shrink-0 flex items-center gap-2" style={{color:'var(--color-brand)'}}>
      ...
      <div className="grid grid-cols-4 gap-3">
        {statItems.map(s => (
          <div key={s.label} className={`bg-white rounded-2xl border ${s.border} p-3 sm:p-4 text-center shadow-sm group transition-all hover:-translate-y-0.5`}>
            <div className={`w-8 h-8 sm:w-9 sm:h-9 ${s.bg} rounded-xl flex items-center justify-center mx-auto mb-1.5 transition-transform ${s.anim}`}>
              <s.Icon className={`w-4 h-4 sm:w-5 sm:h-5 ${s.iconClass}`}/>
            </div>
            <p className="text-xl sm:text-2xl font-display font-bold text-slate-800">{s.value}</p>
      ...
      {past.length > 0 && (
        <section>
          ...
          <div className="space-y-3">
            {past.slice(0,3).map(a => <AppointmentCard key={a.id} appt={a} onCancel={setCancelModal}/>)}
          </div>
          {past.length > 3 && (
            <Link to="/dashboard/appointments"
              className="flex items-center justify-center gap-1.5 mt-3 py-3 rounded-2xl text-sm font-semibold text-sky-500 hover:text-sky-600 hover:bg-sky-50 transition-all border border-sky-100">
              View all {past.length} past appointments <ChevronRight className="w-4 h-4"/>
            </Link>
          )}
        </section>
      )}
      ...
      {cancelled.length > 0 && (
        <section>
          <button onClick={() => setShowCancelled(p => !p)} className="w-full flex items-center justify-between mb-3">
            ...
            <ChevronRight className={`w-4 h-4 text-slate-400 transition-transform ${showCancelled ? 'rotate-90' : ''}`}/>
          </button>
          {showCancelled && (
